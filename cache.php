...
class cache
{
	...
	function connect(array $servers, $failure_callback = NULL)
	{
		...
		foreach ($servers as $server)
		{
			...
			{
				$this->memcache->addServer($server['host'], $server['port'], $persistent, $weight, NULL, NULL, NULL, $failure_callback);
			}
			
			if ($this->library == 'memcached')
			{
				$this->memcache->addServer($server['host'], $server['port'], $weight);
			}
		}
	}
